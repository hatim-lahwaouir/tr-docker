...
def get_conversations(request):
    ...
    all_seen = not Messages.objects.filter(receiver=user_id, seen=False).exists()
    return Response({'conversations' :s.data , 'all_seen' : all_seen}, status=status.HTTP_200_OK)
