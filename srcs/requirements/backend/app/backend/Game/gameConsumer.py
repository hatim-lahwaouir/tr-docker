...
def setInGame(user, state):
    user.isInGame = state
    print(f"{user.username} IS IN GAAAAAAAAAAAAAAAME")
    user.save()
...
class consumer(AsyncWebsocketConsumer):
    ...
    def winner(self):
        ...
        print(f"{player.username}: is WINNER")
        player.wins += 1
        ...
        if self.gc.typeOfMatch == 'tr':
            ...
            if (player.PositionR1 == 1 or player.PositionR1 == 2) and player.trRound < 3:
                ...
                try:
                    ...
                    print(f"\033[1;31m {m} created in try I roundofPlayer({player.trRound})\033[1;0m")
                    m.save()
                    player.games += 1
                    player.save()
                    o.games += 1
                ...
                try:
                    ...
                    m = Match.objects.create(player=player, opponent=o, typeOfMatch='tr')
                    m.tournament = self.gc.tournament
                    print(f"\033[1;31m {m} created in try II roundofPlayer({player.trRound})\033[1;0m")
                    m.save()
                    player.games += 1
                    player.save()
                    o.games += 1
...
#             return
#         player = self.gc.winner
#         print(f"{self.user} says that: {player.username}: is WINNER {player.PositionR1}")
#         player.wins += 1
#         player.level += Decimal('0.56')
#         player.fract_level = (player.level % 1) * 100
#         if self.gc.typeOfMatch == 'tr':
...
#         self.gc.player.isInGame = False
#         self.gc.opponent.isInGame = False
#         # self.gc.player.ready = False
#         # self.gc.opponent.ready = False
#         self.gc.finished = True
#         self.gc.player.save()
#         self.gc.opponent.save()
#         self.gc.save()
#         print(f'{self.gc}:==============>FINISHED')
...
#             gameD[gId]['ball_x'] = w / 2
#             gameD[gId]['ball_y'] = h / 2
#             gameD[gId]['ball_speed_x'] = 5 * random.choice([-1, 1])
#             gameD[gId]['ball_speed_y'] = random.uniform(-2, 2)
#             gameD[gId]['ball_speed'] = 5
#             gameD[gId]['scored'] = 0
